...
LedRgbStatus::LedRgbStatus(uint8_t pin, uint8_t brightness, uint16_t pixelsCount)
    : _pin(pin),
...
void LedRgbStatus::statusBoot() {
    setBlink(Color::Blue(), 150, 150);
}

void LedRgbStatus::statusWifiConnecting() {
    setPulse(Color::Yellow(), 1200);
}

void LedRgbStatus::statusOnline() {
    setSolid(Color::Green());
}

void LedRgbStatus::statusWarn() {
    setBlink(Color::Cyan(), 250, 750);
}

void LedRgbStatus::statusError() {
    setBlink(Color::Red(), 120, 120);
}
